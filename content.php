...
$sql='SELECT `d`.* ,`u`.`name`,`u`.`img_name`,`u`.`created`FROM`diary`AS`d`LEFT JOIN`users` AS `u` ON `d`.`user_id` = `u`.`id`WHERE `d`.`id`=?' ;
$data=[$content_id];
$stmt=$dbh->prepare($sql);
$stmt->execute($data);
  ...
  <div class="container-fluid">
    ...
    <div class="row " style="text-align: center">
      <form method="post" action="edit.php" enctype="multipart/form-data">
          ...
          <a href="register/login.php"class="btn btn-sm btn-info center-block>
